...
import { C as Plus, P as MapPin, R as LoaderCircle, S as Printer, T as Pencil, Z as Download, c as Trash2, x as QrCode } from "../_libs/lucide-react.mjs";
...
import { t as Route } from "./admin.vi-tri-OANAIol3.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/admin.vi-tri-B9G_XUQx.js
...
function TablesPage() {
	...
	const [branchFilter, setBranchFilter] = (0, import_react.useState)(search.store_id ?? "all");
	const [loading, setLoading] = (0, import_react.useState)(true);
	const [qrMap, setQrMap] = (0, import_react.useState)({});
	const [dialogOpen, setDialogOpen] = (0, import_react.useState)(false);
	const [editing, setEditing] = (0, import_react.useState)(null);
	const [deleting, setDeleting] = (0, import_react.useState)(null);
	const [formNum, setFormNum] = (0, import_react.useState)("1");
	...
	(0, import_react.useEffect)(() => {
		apiGet("/admin/branches").then(setBranches).catch(() => {});
	}, []);
	function tableNumber(name) {
		const m = name.match(/(\d+)/);
		return m ? Number(m[1]) : 0;
	}
	function nextTableNumber(storeId) {
		const nums = tables.filter((t) => String(t.store_id) === storeId).map((t) => tableNumber(t.name)).filter((n) => n > 0);
		return (nums.length ? Math.max(...nums) : 0) + 1;
	}
	const filtered = branchFilter === "all" ? tables : tables.filter((t) => t.store_id === Number(branchFilter));
	function openCreate() {
		setEditing(null);
		const store = branchFilter === "all" ? branches.length ? String(branches[0].id) : "1" : branchFilter;
		setFormStore(store);
	...
	async function save() {
		...
		if (!Number.isInteger(num) || num <= 0) return toast.error("Số bàn phải là số nguyên dương (1, 2, 3...)");
		const name = `Bàn ${num}`;
		if (tables.some((x) => String(x.store_id) === formStore && x.id !== editing?.id && tableNumber(x.name) === num)) return toast.error(`Số bàn ${num} đã tồn tại trong chi nhánh này`);
		setSaving(true);
		try {
			if (editing) {
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							...
							children: [
								...
								/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
									...
									children: [
										"Sẽ tạo bàn mang tên ",
										/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("strong", { children: ["Bàn ", formNum || "..."] }),
										" — không được trùng số trong chi nhánh."
									]
								})
							]
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
