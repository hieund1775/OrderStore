...
function Checkout() {
  ...
  async function submitOrder() {
    ...
    // Auto Title Case: tự động viết hoa chữ đầu mỗi từ (xử lý tên Google 1 từ hoặc viết thường)
    ...
    // Tên 1 từ: chấp nhận cho tài khoản Google (backend cũng đã nới lỏng)
    const singleWordRegex = /^[A-Z\u00C0-\u00FF\u0102\u0103\u0110\u0111\u01A0\u01A1\u01AF\u01B0\u1EA0-\u1EF9][a-z\u00C0-\u00FF\u0102\u0103\u0110\u0111\u01A0\u01A1\u01AF\u01B0\u1EA0-\u1EF9]*$/;

    if (!cleanName || (!vnNameRegex.test(cleanName) && !singleWordRegex.test(cleanName))) {
      return toast.error("Họ và tên không hợp lệ (tối thiểu 2 từ, viết hoa chữ cái đầu). Gợi ý: nhập đầy đủ họ và tên.");
    }
