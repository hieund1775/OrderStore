...
export function validateInventoryMovementInput(input) {
  if (!input || typeof input !== 'object') {
    throw new Error('Payload inventory movement phải là một object');
  }
  ...
  if (!['receive', 'adjust', 'reserve', 'release', 'sale', 'cancel_restock', 'return_restock'].includes(movementType)) {
    throw new Error('movement_type không hợp lệ (hỗ trợ receive, adjust, reserve, release, sale, cancel_restock, return_restock)');
  }

  const quantity = Number(input.quantity);
  if (!Number.isInteger(quantity) || quantity === 0) {
    throw new Error('Số lượng quantity phải là số nguyên khác 0');
  }

  const reason = String(input.reason || '').trim();
  if (!reason || reason.length < 2) {
    throw new Error('Vui lòng nhập lý do điều chỉnh tồn kho (reason)');
  }
