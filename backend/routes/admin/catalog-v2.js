...
// =============================================================
// OPTION PRESETS (BLOCK 3 - RETIRED)
// =============================================================
router.get('/presets', requireRole('super', 'manager'), (req, res) => {
  res.status(410).json({ error: 'BLOCK_3_RETIRED', message: 'Block 3 presets have been retired' });
});

router.put('/presets', requireRole('super'), (req, res) => {
  res.status(410).json({ error: 'BLOCK_3_RETIRED', message: 'Block 3 presets have been retired' });
});

router.delete('/presets/:targetType/:targetId/:attributeId', requireRole('super'), (req, res) => {
  res.status(410).json({ error: 'BLOCK_3_RETIRED', message: 'Block 3 presets have been retired' });
});
