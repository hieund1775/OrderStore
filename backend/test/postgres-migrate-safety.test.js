...
async function withGuardEnvironment(values, callback) {
  const keys = ['NODE_ENV', 'POSTGRES_INTEGRATION', 'POSTGRES_TEST_ALLOWED_HOSTS'];
...
describe('PostgreSQL migration safety guard', () => {
  it('denies an unapproved target before Pool.connect', async () => {
    ...
    await withGuardEnvironment({ NODE_ENV: 'test', POSTGRES_INTEGRATION: '1', POSTGRES_TEST_ALLOWED_HOSTS: '' }, async () => {
  ...
  it('denies missing confirmation and a non-test environment before Pool.connect', async () => {
    ...
    await withGuardEnvironment({ NODE_ENV: 'test', POSTGRES_INTEGRATION: '0', POSTGRES_TEST_ALLOWED_HOSTS: '' }, async () => {
      await assert.rejects(
    ...
    await withGuardEnvironment({ NODE_ENV: 'development', POSTGRES_INTEGRATION: '1', POSTGRES_TEST_ALLOWED_HOSTS: '' }, async () => {
      await assert.rejects(
  ...
  it('allows a dedicated test database and an explicitly allowlisted host', async () => {
    await withGuardEnvironment({ NODE_ENV: 'test', POSTGRES_INTEGRATION: '1', POSTGRES_TEST_ALLOWED_HOSTS: '' }, async () => {
      ...
      const { result: results, logs } = await captureMigrationLogs(() => runMigrations({
        customUrl: 'postgresql://user:secret@db.example.test:5432/teaplus_test', pool: fake.pool,
      }));
      assert.ok(results.length > 0);
      assert.equal(fake.getConnectCalls(), 1);
      const output = logs.join('\n');
      assert.match(output, /Target DB: host=db\.example\.test, database=teaplus_test/);
      assert.equal(output.includes('user:secret'), false);
      assert.equal(output.includes('?'), false);
    });

    await withGuardEnvironment({ NODE_ENV: 'test', POSTGRES_INTEGRATION: '1', POSTGRES_TEST_ALLOWED_HOSTS: 'db.staging.example' }, async () => {
      const fake = createFakePool();
      const { result: results } = await captureMigrationLogs(() => runMigrations({
        customUrl: 'postgresql://user:secret@db.staging.example:5432/postgres', pool: fake.pool,
      }));
      assert.ok(results.length > 0);
      assert.equal(fake.getConnectCalls(), 1);
    });
