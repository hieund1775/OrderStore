...
export default function StaffPosScreen() {
  ...
  const handleAddToCart = () => {
    ...
    const newItem: CartItem = {
      ...
      product_id: Number(customizingProduct.id),
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const assignedOrderCode = res?.order_code || res?.order_id;
      if (!assignedOrderCode) throw new Error('Máy chủ không trả về mã đơn hợp lệ.');
      ...
      if (isVietQr || res?.qr_code) {
        ...
        if (!qrUrl) throw new Error('Máy chủ không trả về mã QR hợp lệ.');
        setCheckoutQrModal({
          qrUrl,
      ...
      Alert.alert('Không thể tạo đơn', err?.message || 'Vui lòng kiểm tra kết nối và thử lại.');
    } finally {
          ...
          >
            <View style={styles.dropdownLeft}>
              ...
              <Text
                style={[
                  styles.dropdownText,
                  Boolean(selectedTableId || customTableNumber) && styles.dropdownTextActive,
                ]}
                numberOfLines={1}
              >
                      ...
                      <View style={styles.dropdownLeft}>
                        ...
                        <Text
                          style={[
                            ...
                            Boolean(selectedTableId || customTableNumber) && styles.tableDrawerDropdownTextActive,
                          ]}
                          numberOfLines={1}
                        >
