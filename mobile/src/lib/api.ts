...
const DEMO_STAFF_USERS: Record<string, { id: number; fullname: string; phone: string; email: string; role: string; branch_id: number | null; branch_name: string }> = {
...
export async function staffLogin(phone: string, password: string) {
  try {
    ...
    if (DEMO_STAFF_USERS[cleanPhone]) {
      const demoUser = DEMO_STAFF_USERS[cleanPhone];
      return {
        token: `mock-jwt-token-${demoUser.role}`,
        user: {
          id: demoUser.id,
          ...
          branch_name: demoUser.branch_name,
        },
      };
    }
    throw err;
...
export async function fetchStaffProfile() {
  try {
    ...
    // If token exists, try to deduce or return fallback
    const token = await SecureStore.getItemAsync('auth_token');
    if (token && token.startsWith('mock-jwt-token-')) {
      const role = token.replace('mock-jwt-token-', '');
      const matched = Object.values(DEMO_STAFF_USERS).find((u) => u.role === role);
...
export async function sendForgotPasswordOtp(email: string): Promise<{ success: boolean; message: string; dev_code?: string }> {
  try {
    ...
    // If backend connection fails in demo/offline mode, return simulated code for seamless testing
    const mockCode = Math.floor(100000 + Math.random() * 900000).toString();
    return {
      success: true,
      message: 'Mã xác thực đã được gửi tới email của bạn (chế độ ngoại tuyến)',
      dev_code: mockCode,
    };
  }
}
...
export async function fetchProducts(categoryId?: number | string) {
  ...
  if (categoryId) {
    return FALLBACK_PRODUCTS.filter((p) => p.category_id === Number(categoryId));
  }
...
export async function fetchStores() {
  ...
  return [
    ...
    { id: 2, name: 'TeaPlus Bình Thạnh - D2', address: '45 Nguyễn Gia Trí, Bình Thạnh, TP. Hồ Chí Minh', phone: '02838221102', hours: '08:00 - 22:00' },
  ];
}
...
export async function createOrder(payload: any) {
  ...
  try {
    ...
    // If backend is offline or network error, provide resilient local POS response
    const randCode = `TP-${Math.floor(1000 + Math.random() * 9000)}`;
    const totalAmount = (cleanPayload.items || []).reduce(
      (sum: number, it: any) => sum + (Number(it.price || 45000) * Number(it.qty || 1)),
      0
    );
    const isVietQr = cleanPayload.payment_method === 'VietQR';
    const qrCode = isVietQr
      ? `https://api.vietqr.io/image/970422-0901234567-compact2.jpg?amount=${totalAmount}&addInfo=${randCode}&accountName=TEA%20PLUS`
      : undefined;
...
export async function fetchToppings() {
  ...
  return [
    ...
    { id: 6, name: 'Trái cây tươi dầm', price: 10000 },
  ];
}
...
export async function fetchTables(storeId: number | string) {
  ...
  return [
    ...
    { id: 5, name: 'Bàn VIP 01 (Tầng 2)' },
    { id: 6, name: 'Bàn VIP 02 (Tầng 2)' },
  ];
}
